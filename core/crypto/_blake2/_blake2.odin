...
init_odin :: proc(ctx: ^$T) {
	...
	if ctx.cfg.tree != nil {
		...
		p[3] = ctx.cfg.tree.(Blake2_Tree).maxDepth
		put_u32_le(p[4:], ctx.cfg.tree.(Blake2_Tree).leafSize)
		when T == Blake2s_Context {
			p[8]  = byte(ctx.cfg.tree.(Blake2_Tree).nodeOffset)
			p[9]  = byte(ctx.cfg.tree.(Blake2_Tree).nodeOffset >> 8)
			p[10] = byte(ctx.cfg.tree.(Blake2_Tree).nodeOffset >> 16)
			p[11] = byte(ctx.cfg.tree.(Blake2_Tree).nodeOffset >> 24)
			p[12] = byte(ctx.cfg.tree.(Blake2_Tree).nodeOffset >> 32)
			p[13] = byte(ctx.cfg.tree.(Blake2_Tree).nodeOffset >> 40)
			p[14] = ctx.cfg.tree.(Blake2_Tree).nodeDepth
			p[15] = ctx.cfg.tree.(Blake2_Tree).innerHashSize
		} else when T == Blake2b_Context {
			put_u64_le(p[8:], ctx.cfg.tree.(Blake2_Tree).nodeOffset)
			p[16] = ctx.cfg.tree.(Blake2_Tree).nodeDepth
			p[17] = ctx.cfg.tree.(Blake2_Tree).innerHashSize
		}
	} else {
		p[2], p[3] = 1, 1
	}
	...
	if ctx.cfg.tree != nil && ctx.cfg.tree.(Blake2_Tree).isLastNode {
		ctx.isLastNode = true
	}
	if len(ctx.cfg.key) > 0 {
		copy(ctx.paddedKey[:], ctx.cfg.key)
		update_odin(ctx, ctx.paddedKey[:])
		ctx.isKeyed = true
	}
	copy(ctx.ih[:], ctx.h[:])
	copy(ctx.h[:],  ctx.ih[:])
...
blake2s_final_odin :: proc(ctx: $T, hash: []byte) {
	if ctx.isKeyed {
		for i := 0; i < len(ctx.paddedKey); i += 1 {
			ctx.paddedKey[i] = 0
		}
	}
