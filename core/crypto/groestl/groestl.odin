...
@(private)
_create_groestl_ctx :: #force_inline proc(size: ctx.Hash_Size) {
    ctx: Groestl_Context
...
MUL2 :: #force_inline proc (b: byte) -> byte {
    return (b >> 7) != 0 ? (b << 1) ~ 0x1b : (b << 1)
}

MUL3 :: #force_inline proc (b: byte) -> byte {
    return MUL2(b) ~ b
}

MUL4 :: #force_inline proc (b: byte) -> byte {
    return MUL2(MUL2(b))
}

MUL5 :: #force_inline proc (b: byte) -> byte {
    return MUL4(b) ~ b
}

MUL6 :: #force_inline proc (b: byte) -> byte {
    return MUL4(b) ~ MUL2(b)
}

MUL7 :: #force_inline proc (b: byte) -> byte {
