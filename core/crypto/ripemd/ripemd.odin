...
// hash from its contents
hash_stream_320 :: proc(s: io.Stream) -> ([40]byte, os.Errno) {
    _create_ripemd_ctx(40)
...
// path and pass it to hash_stream to compute a hash
hash_file_320 :: proc(path: string) -> ([40]byte, os.Errno) {
    _create_ripemd_ctx(40)
    return _hash_impl->hash_file_40(path)
}
...
hash_bytes_odin_40 :: #force_inline proc(ctx: ^ctx.Hash_Context, data: []byte) -> [40]byte {
    hash: [40]byte
...
hash_stream_odin_40 :: #force_inline proc(ctx: ^ctx.Hash_Context, fs: io.Stream) -> ([40]byte, os.Errno) {
    hash: [40]byte
    if c, ok := ctx.internal_ctx.(Ripemd320_Context); ok {
        init_odin(&c)
        buf := make([]byte, 512)
...
hash_file_odin_40 :: #force_inline proc(ctx: ^ctx.Hash_Context, path: string) -> ([40]byte, os.Errno) {
    if hd, err := os.open(path); err == os.ERROR_NONE {
