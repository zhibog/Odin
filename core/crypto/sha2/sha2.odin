...
hash_bytes_odin_28 :: #force_inline proc(ctx: ^ctx.Hash_Context, data: []byte) -> [SHA224_DIGEST_SIZE]byte {
...
hash_stream_odin_28 :: #force_inline proc(ctx: ^ctx.Hash_Context, fs: io.Stream) -> ([SHA224_DIGEST_SIZE]byte, os.Errno) {
    hash: [SHA224_DIGEST_SIZE]byte
    if c, ok := ctx.internal_ctx.(Sha256_Context); ok {
        init_odin(&c)
        buf := make([]byte, 512)
...
hash_file_odin_28 :: #force_inline proc(ctx: ^ctx.Hash_Context, path: string) -> ([SHA224_DIGEST_SIZE]byte, os.Errno) {
    if hd, err := os.open(path); err == os.ERROR_NONE {
        return hash_stream_odin_28(ctx, os.stream_from_handle(hd))
    } else {
        return [SHA224_DIGEST_SIZE]byte{}, err
    }
}

hash_bytes_odin_32 :: #force_inline proc(ctx: ^ctx.Hash_Context, data: []byte) -> [SHA256_DIGEST_SIZE]byte {
...
hash_stream_odin_32 :: #force_inline proc(ctx: ^ctx.Hash_Context, fs: io.Stream) -> ([SHA256_DIGEST_SIZE]byte, os.Errno) {
    hash: [SHA256_DIGEST_SIZE]byte
    if c, ok := ctx.internal_ctx.(Sha256_Context); ok {
        init_odin(&c)
        buf := make([]byte, 512)
...
hash_file_odin_32 :: #force_inline proc(ctx: ^ctx.Hash_Context, path: string) -> ([SHA256_DIGEST_SIZE]byte, os.Errno) {
    if hd, err := os.open(path); err == os.ERROR_NONE {
        return hash_stream_odin_32(ctx, os.stream_from_handle(hd))
    } else {
        return [SHA256_DIGEST_SIZE]byte{}, err
    }
}

hash_bytes_odin_48 :: #force_inline proc(ctx: ^ctx.Hash_Context, data: []byte) -> [SHA384_DIGEST_SIZE]byte {
...
hash_stream_odin_48 :: #force_inline proc(ctx: ^ctx.Hash_Context, fs: io.Stream) -> ([SHA384_DIGEST_SIZE]byte, os.Errno) {
    hash: [SHA384_DIGEST_SIZE]byte
    if c, ok := ctx.internal_ctx.(Sha512_Context); ok {
        init_odin(&c)
        buf := make([]byte, 512)
...
hash_file_odin_48 :: #force_inline proc(ctx: ^ctx.Hash_Context, path: string) -> ([SHA384_DIGEST_SIZE]byte, os.Errno) {
    if hd, err := os.open(path); err == os.ERROR_NONE {
        return hash_stream_odin_48(ctx, os.stream_from_handle(hd))
    } else {
        return [SHA384_DIGEST_SIZE]byte{}, err
    }
}

hash_bytes_odin_64 :: #force_inline proc(ctx: ^ctx.Hash_Context, data: []byte) -> [SHA512_DIGEST_SIZE]byte {
...
hash_stream_odin_64 :: #force_inline proc(ctx: ^ctx.Hash_Context, fs: io.Stream) -> ([SHA512_DIGEST_SIZE]byte, os.Errno) {
    hash: [SHA512_DIGEST_SIZE]byte
    if c, ok := ctx.internal_ctx.(Sha512_Context); ok {
        init_odin(&c)
        buf := make([]byte, 512)
...
hash_file_odin_64 :: #force_inline proc(ctx: ^ctx.Hash_Context, path: string) -> ([SHA512_DIGEST_SIZE]byte, os.Errno) {
    if hd, err := os.open(path); err == os.ERROR_NONE {
        return hash_stream_odin_64(ctx, os.stream_from_handle(hd))
    } else {
        return [SHA512_DIGEST_SIZE]byte{}, err
    }
}
...
final_odin :: proc(ctx: ^$T, hash: []byte) {
	...
	when T == Sha256_Context {
		if ctx.is224 {
			for i = 0; i < 7; i += 1 {UNPACK32(hash[i << 2:], ctx.h[i])}
		} else {
			for i = 0; i < 8; i += 1 {UNPACK32(hash[i << 2:], ctx.h[i])}
		} 
	} else when T == Sha512_Context {
		if ctx.is384 {
			for i = 0; i < 6; i += 1 {UNPACK64(hash[i << 3:], ctx.h[i])}
		} else {
			for i = 0; i < 8; i += 1 {UNPACK64(hash[i << 3:], ctx.h[i])}
		} 
	} 
}