...
test_md2 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := md2.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_md4 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := md4.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    md4.use_botan()
...
test_md5 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := md5.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    md5.use_botan()
...
test_sha1 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha1.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha1.use_botan()
...
test_sha224 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha2.hash_224(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha2.use_botan()
...
test_sha256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha2.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha2.use_botan()
...
test_sha384 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha2.hash_384(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha2.use_botan()
...
test_sha512 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha2.hash_512(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha2.use_botan()
...
test_sha3_224 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha3.hash_224(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha3.use_botan()
...
test_sha3_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha3.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha3.use_botan()
...
test_sha3_384 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha3.hash_384(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha3.use_botan()
...
test_sha3_512 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sha3.hash_512(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha3.use_botan()
...
test_shake_128 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := shake.hash_128(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha3.use_botan()
...
test_shake_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := shake.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sha3.use_botan()
...
test_keccak_224 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := keccak.hash_224(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    keccak.use_botan()
...
test_keccak_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := keccak.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    keccak.use_botan()
...
test_keccak_384 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := keccak.hash_384(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    keccak.use_botan()
...
test_keccak_512 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := keccak.hash_512(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    keccak.use_botan()
...
test_whirlpool :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := whirlpool.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    whirlpool.use_botan()
...
test_gost :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := gost.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    gost.use_botan()
...
test_streebog_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := streebog.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    streebog.use_botan()
...
test_streebog_512 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := streebog.hash_512(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    streebog.use_botan()
...
test_blake2b :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := blake2b.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    blake2b.use_botan()
...
test_blake2s :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := blake2s.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_ripemd_128 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := ripemd.hash_128(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_ripemd_160 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := ripemd.hash_160(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    ripemd.use_botan()
...
test_ripemd_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := ripemd.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_ripemd_320 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := ripemd.hash_320(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_tiger_128 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := tiger.hash_128(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    tiger.use_botan()
...
test_tiger_160 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := tiger.hash_160(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    tiger.use_botan()
...
test_tiger_192 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := tiger.hash_192(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    tiger.use_botan()
...
test_tiger2_128 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := tiger2.hash_128(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_tiger2_160 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := tiger2.hash_160(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_tiger2_192 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := tiger2.hash_192(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_sm3 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := sm3.hash(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    sm3.use_botan()
...
test_skein512 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := skein.hash_skein512(transmute([]byte)(v.str), 64)
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    skein.use_botan()
...
test_jh_224 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := jh.hash_224(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_jh_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := jh.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_jh_384 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := jh.hash_384(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_jh_512 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := jh.hash_512(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_groestl_224 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := groestl.hash_224(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_groestl_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := groestl.hash_256(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_groestl_384 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := groestl.hash_384(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_groestl_512 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors {
        computed     := groestl.hash_512(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_haval_128 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors_3 {
        computed     := haval.hash_128_3(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    ...
    for v, _ in test_vectors_4 {
        computed     := haval.hash_128_4(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    ...
    for v, _ in test_vectors_5 {
        computed     := haval.hash_128_5(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_haval_160 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors_3 {
        computed     := haval.hash_160_3(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_4 := [?]TestHash {
    ...
    for v, _ in test_vectors_4 {
        computed     := haval.hash_160_4(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_5 := [?]TestHash {
    ...
    for v, _ in test_vectors_5 {
        computed     := haval.hash_160_5(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_haval_192 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors_3 {
        computed     := haval.hash_192_3(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_4 := [?]TestHash {
    ...
    for v, _ in test_vectors_4 {
        computed     := haval.hash_192_4(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_5 := [?]TestHash {
    ...
    for v, _ in test_vectors_5 {
        computed     := haval.hash_192_5(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_haval_224 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors_3 {
        computed     := haval.hash_224_3(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_4 := [?]TestHash {
    ...
    for v, _ in test_vectors_4 {
        computed     := haval.hash_224_4(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_5 := [?]TestHash {
    ...
    for v, _ in test_vectors_5 {
        computed     := haval.hash_224_5(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}
...
test_haval_256 :: proc(t: ^testing.T) {
    ...
    for v, _ in test_vectors_3 {
        computed     := haval.hash_256_3(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_4 := [?]TestHash {
    ...
    for v, _ in test_vectors_4 {
        computed     := haval.hash_256_4(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
    test_vectors_5 := [?]TestHash {
    ...
    for v, _ in test_vectors_5 {
        computed     := haval.hash_256_5(transmute([]byte)(v.str))
        computed_str := hex_string(computed[:])
        expect(t, check_hash(computed_str, v.hash), fmt.tprintf("Expected: %s for input of %s, but got %s instead", v.hash, v.str, computed_str))
    }
}